...
# Nueva funcion para extraer palabras similares



def extract_new_terms(abstracts, keywords_dict, top_n=15):
    """
    Extrae términos más frecuentes de los abstracts que no estén ya en keywords_dict.
    Devuelve una lista con los top_n términos más comunes.
    """
    # Unir todos los abstracts
    all_text = " ".join(abstracts).lower()
    
    # Tokenizar palabras (solo letras y guiones)
    words = re.findall(r"\b[a-zA-Z-]+\b", all_text)

    # Palabras clave ya conocidas
    known_terms = {syn.lower() for terms in keywords_dict.values() for syns in terms.values() for syn in syns}

    # Stopwords básicas 
    stopwords = {"the","and","of","to","in","for","with","on","by","is","are","an","at","from","this","that","as","we","our", "a"}
